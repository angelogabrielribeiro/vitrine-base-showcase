...
const CARE_OPTIONS = [
  ...
  },
  {
    ...
    features: [
      ...
      "Monitoramento de funcionamento e backups quando aplicáveis",
    ...
    <div className="min-h-screen overflow-x-clip bg-[#05070a] text-white">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#05070a]/88 backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-4 px-4">
          <Link to="/" className="font-semibold tracking-[0.22em] text-amber-300">
            VITRINE BASE
          </Link>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-xs font-semibold text-cyan-100 transition hover:bg-cyan-300/15"
          >
      ...
      <main>
        <section className="relative isolate overflow-hidden border-b border-white/10 px-4 py-24 sm:py-32">
          <motion.div
            ...
            animate={
              reduce
                ...
                : {
                    background: [
                      "radial-gradient(circle at 18% 25%,rgba(34,211,238,.12),transparent 34%),radial-gradient(circle at 82% 72%,rgba(251,191,36,.10),transparent 32%)",
                      "radial-gradient(circle at 26% 32%,rgba(34,211,238,.18),transparent 39%),radial-gradient(circle at 75% 64%,rgba(251,191,36,.14),transparent 36%)",
                      "radial-gradient(circle at 18% 25%,rgba(34,211,238,.12),transparent 34%),radial-gradient(circle at 82% 72%,rgba(251,191,36,.10),transparent 32%)",
                    ],
                  }
            }
          ...
          <div className="mx-auto max-w-5xl text-center">
            <Link to="/" className="inline-flex items-center gap-2 text-xs text-white/48 hover:text-white/80">
              <ArrowLeft className="h-4 w-4" /> Voltar às demonstrações
            </Link>
            <p className="mt-12 text-xs font-bold uppercase tracking-[0.28em] text-cyan-200">
              Projeto completo · preço claro · sem mensalidade obrigatória
            </p>
            <h1 className="mx-auto mt-6 max-w-5xl font-display text-5xl font-light leading-[0.95] tracking-[-0.055em] sm:text-7xl lg:text-[6rem]">
              Você paga pela construção. Depois escolhe como quer ser acompanhado.
            </h1>
            <p className="mx-auto mt-7 max-w-2xl text-base leading-8 text-white/60 sm:text-lg">
              Domínio .com.br do primeiro ano, configuração e publicação entram no projeto. A manutenção mensal é uma escolha, não uma condição para receber o site.
            </p>
            <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <a
                href="#criacao"
                className="inline-flex min-h-12 items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-100"
              >
                Comparar projetos <ArrowRight className="h-4 w-4" />
              </a>
            ...
            <Reveal>
              <div className="mx-auto max-w-3xl text-center">
                ...
                <h2 className="mt-4 font-display text-4xl font-light tracking-[-0.04em] sm:text-6xl">
                  A diferença está na complexidade, não em entregar um site pela metade.
                </h2>
                <p className="mt-5 text-sm leading-7 text-white/55 sm:text-base">
                  Todos os níveis incluem direção visual, responsividade, publicação e o primeiro ano de domínio .com.br. O escopo define profundidade, volume e integrações.
                </p>
              </div>
            </Reveal>
              ...
              {CREATION_PLANS.map((plan, index) => (
                ...
                >
                  ...
                  <div className="mt-auto pt-7">
                    <ProposalButton label={`Conversar sobre o ${plan.name}`} />
                  </div>
                </motion.article>
              ))}
            </div>
          ...
          <div className="mx-auto max-w-6xl">
            <Reveal>
              <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
                <div className="max-w-3xl">
                  ...
                  <h2 className="mt-4 font-display text-4xl font-light tracking-[-0.04em] sm:text-6xl">
                    Assinatura opcional. Escolha recorrência pela conveniência, não por obrigação.
                  </h2>
                </div>
                <p className="max-w-md text-sm leading-7 text-white/55">
                  Sem assinatura, alterações são cobradas sob demanda. Nos planos mensais, a franquia não acumula e correções de defeitos da implementação não consomem horas.
                </p>
              </div>
            </Reveal>

            <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {CARE_OPTIONS.map((plan, index) => (
                <motion.article
                  key={plan.name}
                ...
                >
                  ...
                  <div className="my-5 border-y border-white/10 py-5">
                    ...
                    <p className="mt-1 text-xs text-white/42">{plan.price === "0" ? "por mês" : "por mês"}</p>
            ...
            <div className="mt-7 grid gap-4 rounded-3xl border border-white/10 bg-white/[0.035] p-6 md:grid-cols-3">
              <div className="flex gap-3">
                ...
                <div><h3 className="font-semibold">Blocos de 15 minutos</h3><p className="mt-1 text-sm leading-6 text-white/50">O relatório mostra somente o tempo efetivamente trabalhado.</p></div>
              </div>
              <div className="flex gap-3">
                <Wrench className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" />
                <div><h3 className="font-semibold">R$ 150 por hora excedente ou avulsa</h3><p className="mt-1 text-sm leading-6 text-white/50">Qualquer excedente é executado somente depois da aprovação.</p></div>
              </div>
              <div className="flex gap-3">
                <ServerCog className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" />
                <div><h3 className="font-semibold">Novas funcionalidades são projeto</h3><p className="mt-1 text-sm leading-6 text-white/50">A franquia cobre suporte e ajustes; módulos novos recebem escopo próprio.</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="extras" className="scroll-mt-24 border-y border-white/10 bg-[#071018] px-4 py-24">
          <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[0.82fr_1.18fr]">
            <Reveal>
              <p className="text-xs font-bold uppercase tracking-[0.26em] text-cyan-200">O que pode ficar fora do valor</p>
              <h2 className="mt-4 font-display text-4xl font-light tracking-[-0.04em] sm:text-5xl">
                Só custo externo que realmente depende da escolha ou do consumo do cliente.
              </h2>
              <p className="mt-5 text-sm leading-7 text-white/55">
                A Vitrine Base inclui o trabalho de configurar e integrar o que estiver no escopo. Quando um fornecedor cobra licença, tarifa ou consumo recorrente, isso é informado antes da aprovação e, sempre que possível, fica na conta do próprio cliente.
              </p>
            </Reveal>
            <Reveal delay={0.08}>
              <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-6 sm:p-8">
                ...
                <div className="mt-7 rounded-2xl border border-amber-300/20 bg-amber-300/[0.055] p-5 text-sm leading-6 text-amber-50/78">
                  <strong className="block text-amber-200">Domínio comum não entra nessa lista.</strong>
                  O primeiro ano de um domínio .com.br disponível já está incluído no preço de criação. Extensões premium ou nomes com custo especial têm somente a diferença informada ao cliente.
                </div>
              </div>
            </Reveal>
          </div>
        ...
        <section className="px-4 py-24 text-center">
          <div className="mx-auto max-w-4xl">
            <p className="text-xs font-bold uppercase tracking-[0.26em] text-amber-200">Próximo passo</p>
            <h2 className="mt-4 font-display text-4xl font-light tracking-[-0.045em] sm:text-6xl">
              Escolha o tamanho do projeto. A assinatura vem depois — somente se fizer sentido.
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-white/55 sm:text-base">
              Antes de começar, páginas, produtos, integrações, prazo e qualquer custo externo ficam registrados na proposta. Sem surpresa depois da entrega.
            </p>
            <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <ProposalButton label="Solicitar proposta pelo WhatsApp" />
              <a
                href={INSTAGRAM_URL}
                target="_blank"
                rel="noreferrer"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-6 py-3 text-sm font-semibold text-white/72 transition hover:bg-white/[0.08] hover:text-white"
              >
                Ver projetos no Instagram <Instagram className="h-4 w-4" />
              </a>
            </div>
          </div>
        </section>
      ...
      <footer className="border-t border-white/10 px-4 py-8 text-xs text-white/38">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <span>Vitrine Base · sites e webapps para negócios locais</span>
          <span>Valores finais dependem do escopo aprovado em proposta.</span>
        </div>
      </footer>
    </div>
  );
