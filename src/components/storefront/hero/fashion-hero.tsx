...
export function FashionHero({ store, spotlight, featured }: NicheHeroProps) {
  ...
  const [i0, i1, i2, i3] = [0, 1, 2, 3].map((i) => uniq[i % Math.max(uniq.length, 1)] ?? banner?.image ?? "");
  const price =
  ...
  const par = (mx: number, my: number) =>
    reduce ? undefined : { transform: `translate3d(${pt.x * mx}px, ${pt.y * my}px, 0)` };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "radial-gradient(closest-side, oklch(0.62 0.22 20 / 0.55), transparent)" }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-32 bottom-0 h-[36rem] w-[36rem] rounded-full opacity-70 blur-3xl"
        style={{ background: "radial-gradient(closest-side, oklch(0.55 0.24 340 / 0.55), transparent)" }}
      />
      <div
        aria-hidden
...
function CollageImage({
  ...
  return (
    <motion.div
      ...
      style={{ transition: "transform 0.5s cubic-bezier(.22,1,.36,1)", ...(style ?? {}) }}
