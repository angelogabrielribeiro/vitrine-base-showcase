...
function OrderDetail() {
  ...
  const trackingStopped = order.status === "cancelado" || order.status === "reembolsado";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          {store.niche === "restaurant" ? (
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <Badge variant="secondary">{orderStatusLabel(effectiveStatus)}</Badge>
                {!trackingStopped && (
            ...
            <div>
              ...
              <Select
                value={order.status}
                onValueChange={(v) => {
                  repo.updateOrderStatus(storeSlug, order.id, v as OrderStatus);
          ...
          <Button asChild variant="outline">
            <a
              href={waCustomerFromAdmin(store, order.customer.whatsapp, order.number)}
              target="_blank"
              rel="noreferrer"
            >
          ...
          {store.niche === "restaurant" &&
            (trackingStopped ? (
              <Button
                variant="outline"
                onClick={() => {
                  repo.updateOrderStatus(storeSlug, order.id, "pago");
                  toast.success("Fluxo automático retomado");
                }}
              >
              ...
              <Button
                variant="outline"
                onClick={() => {
                  repo.updateOrderStatus(storeSlug, order.id, "cancelado");
      ...
      {store.niche === "restaurant" && (
        <>
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-950 dark:text-emerald-100">
            Este pedido avança automaticamente no showcase. O operador só intervém em exceções, como
            cancelamento. Em produção, o relógio é substituído por eventos do pagamento, cozinha/PDV
            e entrega.
          </div>
            ...
            {order.items.map((i) => (
              <li key={i.key} className="flex items-center gap-3 py-3">
                ...
                <div className="flex-1">
                  ...
                  {i.notes && (
                    <div className="text-xs italic text-muted-foreground">Obs: {i.notes}</div>
                  )}
                </div>
          ...
          <section className="rounded-[var(--radius)] border border-border bg-card p-5">
            ...
            <div className="mt-2 space-y-1 text-sm">
              ...
              {order.fulfillment.notes && (
                <div className="italic text-muted-foreground">Obs: {order.fulfillment.notes}</div>
              )}
            </div>
          </section>
          <section className="rounded-[var(--radius)] border border-border bg-card p-5">
            ...
            {order.payment.change && (
              <div className="text-xs text-muted-foreground">
                Troco para {brl(order.payment.change)}
              </div>
            )}
          </section>
        </div>
