  ...
  const orders = repo
    ...
    .filter(({ order, effectiveStatus }) => {
      ...
      if (q) {
        ...
        return (
          order.number.toLowerCase().includes(t) ||
          order.customer.name.toLowerCase().includes(t) ||
          order.customer.whatsapp.includes(t)
        );
      }
      return true;
    });
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-[var(--radius)] border border-border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {orders.map(({ order: o, effectiveStatus }) => (
              <tr
                key={o.id}
