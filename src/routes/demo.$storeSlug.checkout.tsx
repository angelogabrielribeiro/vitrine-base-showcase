...
function CheckoutPage() {
  ...
  const availableFulfillments = useMemo(() => {
    ...
    if (store.fulfillment.pickup)
      opts.push({ value: "pickup", label: "Retirada no local", fee: 0 });
    if (store.fulfillment.localDelivery)
      opts.push({ value: "local", label: "Entrega local", fee: store.deliveryFee });
    if (store.fulfillment.shipping)
      opts.push({ value: "shipping", label: "Envio", fee: store.deliveryFee });
    return opts;
  }, [store]);
  ...
  const submit = () => {
    ...
    const order: Order = {
      ...
      payment: {
        method: payment,
        change: payment === "cash" && change ? Number(change) : undefined,
      },
  ...
  if (!hydrated) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="mt-8 h-56 animate-pulse rounded-[var(--radius)] border border-border bg-card motion-reduce:animate-none" />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="mt-4 flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs">
        ...
        <p>
          Demonstração: nenhum pagamento é processado. Dados de cartão não são solicitados nem
          armazenados.
        </p>
      </div>
        ...
        <div className="space-y-8">
          <section className="rounded-[var(--radius)] border border-border bg-card p-5">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Nenhuma conta é necessária. Usaremos estes dados somente para o pedido e as
              atualizações.
            </p>
          ...
          <section className="rounded-[var(--radius)] border border-border bg-card p-5">
            ...
            {store.niche === "restaurant" && (
              <div className="mt-4 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-emerald-800 dark:text-emerald-200">
                Após finalizar, você acompanhará pedido aceito, preparo, saída e entrega em uma tela
                que atualiza automaticamente — sem login.
              </div>
            )}
            ...
            <div className="mt-3">
              ...
              <Textarea
                ...
                placeholder="Instruções para a loja..."
              />
            </div>
          </section>
          ...
          <section className="rounded-[var(--radius)] border border-border bg-card p-5">
            ...
            {(payment === "credit" || payment === "debit") && (
              <div className="mt-4 rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
                <p className="mb-3">
                  Interface visual apenas — na versão do cliente, os dados serão processados por
                  provedor homologado (ex.: Stripe, Mercado Pago). Nenhum dado real é armazenado
                  nesta demonstração.
                </p>
          ...
          {store.checkout.allowCoupon && (
            <section className="rounded-[var(--radius)] border border-border bg-card p-5">
              ...
              {discount > 0 && (
                <p className="mt-2 text-xs text-emerald-600">Desconto aplicado: −{brl(discount)}</p>
              )}
            </section>
          )}
          ...
          <section className="space-y-3 rounded-[var(--radius)] border border-border bg-card p-5">
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>
                Li e aceito os{" "}
                <Link to="/demo/$storeSlug/termos" params={{ storeSlug }} className="underline">
                  termos
                </Link>{" "}
                e a{" "}
                <Link
