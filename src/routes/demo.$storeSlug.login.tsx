...
function LoginPage() {
  ...
  const enter = (label: string) => {
    ...
    navigate({ to: "/demo/$storeSlug/meus-pedidos", params: { storeSlug } });
  };
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-10">
      ...
      <h1 className="font-display mt-2 text-3xl font-semibold">
        {mode === "login" ? "Entrar para sincronizar" : "Criar conta opcional"}
      </h1>
      <p className="mt-1 text-sm text-muted-foreground">{store.name}</p>
      <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
        Você pode comprar e acompanhar pedidos sem cadastro. A conta serve apenas para sincronizar
        histórico, endereços e agendamentos entre dispositivos.
      </p>

      <div className="mt-4 flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs">
        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />
        <p>
          Sessão demonstrativa. Nenhuma credencial é validada; a autenticação real será conectada na
          versão do cliente.
        </p>
      </div>
      ...
      >
        ...
        <Button type="submit" className="w-full">
          {mode === "login" ? "Entrar e sincronizar" : "Criar conta e sincronizar"}
        </Button>
      </form>
